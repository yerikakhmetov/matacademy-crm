...
## Авто-напоминания в WhatsApp (Meta Cloud API, необязательно)

Приоритет каналов: если у родителя привязан Telegram — идёт туда; иначе, если настроен
WhatsApp и есть телефон родителя — идёт в WhatsApp. WhatsApp разрешает бизнес-рассылку
**только по одобренному шаблону**, поэтому шаблон нужно создать и провести модерацию.

1. **Meta for Developers** → создайте приложение типа Business, добавьте продукт
   **WhatsApp**. Получите тестовый номер, **Phone Number ID** и **access token**
   (для прод — постоянный System User token).
2. **Создайте шаблон** (WhatsApp Manager → Message Templates) с **двумя** параметрами в теле:
   - Имя: `payment_reminder`, язык: `ru`, категория: *Utility*
   - Текст тела (пример):
     `Здравствуйте! Напоминание от МатАкадемии по ученику {{1}}: {{2}}. Спасибо!`
   - Отправьте на модерацию, дождитесь статуса **Approved**.
3. **Добавьте переменные** в Vercel:
   - `WHATSAPP_TOKEN` — access token
   - `WHATSAPP_PHONE_ID` — Phone Number ID
   - `WHATSAPP_TEMPLATE` — `payment_reminder` (или своё имя)
   - `WHATSAPP_TEMPLATE_LANG` — `ru`
   Затем **Redeploy**.
4. Проверьте вручную (телефоны родителей должны быть в международном формате):
   ```bash
   curl "https://crm.matacademy.kz/api/cron/reminders?secret=<CRON_SECRET>"
   ```
   В ответе появится `sentWhatsapp`.

> ⚠️ Код отправляет шаблон с параметрами {{1}} = имя ученика, {{2}} = суть напоминания.
> Если сделаете другое число параметров — синхронизируйте с `lib/whatsapp.ts`.
